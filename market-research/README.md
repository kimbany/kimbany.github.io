...
### 1-C. (선택) 쿠팡 파트너스 API 키 — 쿠팡 상품·가격 비교용
1. partners.coupang.com 가입 및 승인
2. 좌측 메뉴 **API 관리** → 액세스키/시크릿키 발급
3. 확보할 값 2개: **ACCESS_KEY**, **SECRET_KEY**
   - 미설정 시 상품·가격 탭의 쿠팡 데이터는 데모값으로 표시됨(네이버는 정상)
...
3. Settings → Variables 에 환경변수 추가 (코드에 넣지 말 것):
   ...
   - (선택) `COUPANG_ACCESS_KEY` · `COUPANG_SECRET_KEY` — 쿠팡 파트너스 API용
