...
가챠 화면만 **라이트 배경 + 두꺼운 유리 + 가장자리 프리즘 분산** 톤을 쓴다.
번호 쟁탈전의 아케이드/네온(다크) 톤과 섞이지 않도록 `gacha.css` 의 모든 규칙은
`.gacha-mode` 아래에만 둔다. 진입 시 `mount()` 가 `<body>` 에 클래스를 붙이고 `unmount()` 가 뗀다.

유리 표현의 재료는 세 가지다.

1. **표면** — 반투명 흰 그라데이션 + `backdrop-filter: blur() saturate()`
2. **가장자리** — `conic-gradient` 를 테두리 두께(1.4px)만큼만 남기고 마스크로 잘라낸 프리즘 링.
   `mask-composite` 를 못 쓰는 브라우저는 `@supports not` 으로 흰 외곽선 대체
3. **그림자** — 접지 그림자 + 넓게 퍼지는 확산 그림자 + `--q-inner` (유리 두께감을 주는 inset)

- 토큰은 `.gacha-mode` 의 CSS 변수로만 정의하고 재사용한다 (`--q-*`).
  색·그림자·반경·이징을 직접 하드코딩하지 말 것
- 폰트: 본문 Pretendard, **숫자·라벨은 모노스페이스(`--q-mono`) + `tabular-nums`**.
  숫자가 흔들리지 않고 IT 톤이 난다
- 한글에 `letter-spacing` 을 0.1em 이상 주면 지나치게 벌어진다. 마이크로 라벨도 0.06~0.08em 까지만
- 배경은 `.g-bg` 레이어(라이트 그라데이션 + 옅은 격자 + 물결 SVG 3겹 + 떠다니는 유리 구슬)를
  mount 때 `<body>` 에 넣고 unmount 때 지운다
- 기계는 `.g-shell` 하나의 유리 본체 안에 창(`.g-dome`) · 조작부(`.g-body-panel`) ·
  배출구(`.g-tray-mouth`)를 넣어 한 덩어리 기기로 읽히게 한다.
  배출구는 라이트 배경에 묻히기 쉬우니 안쪽을 확실히 어둡게 파낸다
- 이모지 금지. 아이콘은 `gacha/icons.js` 의 인라인 SVG (currentColor 상속, stroke 1.7)
- 뷰 레이어(`gacha/app.js`, `icons.js`)는 함수명도 한글로 쓴다.
  단 `gacha/engine.js` 는 테스트·문서와 공유하는 API 라 영문 유지
- **숫자에 임의 등급(레어/에픽/전설)을 붙이지 않는다.** 모든 숫자가 동일 확률인 게
  이 가챠의 보장이라 등급을 매기면 공정성을 오해하게 된다. 특별 연출(프리즘 링 + 파티클)은
  실제로 특별한 "마지막 캡슐"에만 쓴다
